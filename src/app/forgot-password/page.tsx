...
import { Mail, Key, ShieldCheck, ArrowLeft, ArrowRight, CheckCircle2, KeyRound, AlertCircle } from 'lucide-react';
...
export default function ForgotPasswordPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // We pass email and otp to verify it's the right code for the right person
      await apiClient.post('/api/v1/auth/verify-reset-otp', {
        email
      }, {
        params: { otp: fullOtp } // The endpoint expects otp as a query param or part of body? 
        // Wait, I defined it with otp: str in the endpoint...
        // Let's check how FastAPI handles it. 
        // It was: def verify_reset_otp(body: ForgotPasswordRequest, otp: str, ...)
        // So 'otp' is a query param by default in FastAPI if not in the body.
      });
      setStep('PASSWORD');
    } catch (err: any) {
              ...
              >
                <div>
                  ...
                  <div className="relative">
                    <Mail className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-white/70 z-10" />
                    <input
                      type="email"
                      required
                      value={email}
