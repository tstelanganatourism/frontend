...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = isAgent ? [
    ...
    { name: 'My Trips & Tickets', href: '/dashboard/bookings', icon: Ticket },
    { name: 'My Profile', href: '/dashboard/profile', icon: User },
  ];
      ...
      <div className="px-4 py-5 border-b border-slate-100">
        <div className="flex items-center gap-3.5">
          ...
          <div
            ...
            style={{ boxShadow: '0 2px 10px rgba(21,152,161,0.18)' }}
          >
            {user?.avatar_url && !imgError ? (
              ...
              <div
                ...
                style={{ background: 'linear-gradient(135deg, #134e5e, #1598a1)' }}
              >
                {initials}
              </div>
            )}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              {isAgent ? (
                ...
                <BadgeCheck className="h-3 w-3 text-teal-500 shrink-0" />
              )}
              <span className="text-xs font-semibold text-slate-400 truncate">
                {isAgent ? `Agent #${user?.id || ''}` : 'Tourist Account'}
              </span>
            </div>
          </div>
        </div>
          ...
          return (
            <Link
              ...
              className={`group flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 ${
                isActive
                  ? 'bg-[#0e6b74] text-white shadow-sm'
    ...
    <ProtectedRoute allowedRoles={['USER', 'AGENT']}>
      ...
      <div className="md:hidden sticky top-[62px] z-30 bg-white border-b border-slate-100 px-4 py-2.5 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-slate-800 font-bold text-sm leading-tight">{user?.full_name || 'Traveler'}</p>
            <p className="text-slate-400 text-[10px] font-medium leading-tight">{isAgent ? 'Agent Portal' : 'Tourist Portal'}</p>
          </div>
        </div>
        <button
