...
import { Eye, EyeOff, Mail, Lock, User, Phone, Waves, ArrowRight, AlertCircle } from 'lucide-react';
import { touristSignup, getGoogleAuthUrl } from '@/services/authService';
...
export default function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setApiError('Google signup unavailable. Please try again.');
      ...
      >
        ...
        <div className="rounded-3xl border border-white/12 bg-white/8 p-6 backdrop-blur-2xl shadow-[0_32px_80px_rgba(0,0,0,0.45)]">
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/20 bg-white/10 px-4 py-2.5 text-sm font-semibold text-white transition-all duration-200 hover:bg-white/18 disabled:opacity-60"
          >
            {googleLoading ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3" noValidate>
            {/* Full name */}
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-white/60">Full Name</label>
              ...
              {errors.full_name && <p className="mt-1 flex items-center gap-1 text-[10px] text-red-400"><AlertCircle className="h-3 w-3" />{errors.full_name.message}</p>}
            </div>
            ...
            <div>
              ...
              {errors.email && <p className="mt-1 flex items-center gap-1 text-[10px] text-red-400"><AlertCircle className="h-3 w-3" />{errors.email.message}</p>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/15 bg-white/8 py-2.5 pl-10 pr-10 text-sm text-white placeholder-white/30 outline-none ring-0 transition-all focus:border-[#5ac4d7]/70 focus:bg-white/12 focus:ring-2 focus:ring-[#5ac4d7]/20"
                />
                <button type="button" onClick={() => setShowPassword((s) => !s)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/80 transition-colors">
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {errors.password && <p className="mt-1 flex items-center gap-1 text-[10px] text-red-400"><AlertCircle className="h-3 w-3" />{errors.password.message}</p>}
            </div>
            ...
            <AnimatePresence>
              {apiError && (
                <motion.div
                  ...
                  className="flex items-start gap-2.5 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-2.5 text-sm text-red-300"
                >
            ...
            <p className="text-[10px] text-white/35 leading-relaxed pt-1">
              ...
              <Link href="/terms" className="text-[#5ac4d7] hover:text-white transition-colors">Terms of Service</Link>{' '}and{' '}
              <Link href="/terms" className="text-[#5ac4d7] hover:text-white transition-colors">Privacy Policy</Link>.
            </p>

            <button
              type="submit"
              disabled={isSubmitting || googleLoading}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#1a6b7a] py-3 text-sm font-bold text-white shadow-[0_8px_24px_rgba(26,107,122,0.4)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#1d7a8a] hover:shadow-[0_12px_32px_rgba(26,107,122,0.5)] disabled:opacity-60 disabled:translate-y-0 mt-2"
            >
              {isSubmitting ? (
                <svg className="h-5 w-5 animate-spin" viewBox="0 0 24 24" fill="none">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="white" strokeWidth="3"/>
