...
export const CustomDatePicker = ({
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      {label && <label className="block text-xs font-black uppercase tracking-widest text-slate-500 mb-2">{label}</label>}
      <button
        type="button"
        disabled={disabled}
      ...
      {/* DEAD CENTER IN MIDDLE OF SCREEN Overlay via React Portal */}
      {isOpen && isMounted && typeof document !== 'undefined' && createPortal(
        <div
          className="fixed inset-0 z-[999999] flex items-center justify-center bg-slate-900/35 backdrop-blur-xs p-4 animate-in fade-in duration-150"
          onClick={(e) => {
            e.stopPropagation();
            setIsOpen(false);
          }}
        >
          <div
            ref={popoverRef}
            className="w-full max-w-[320px] rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl animate-in zoom-in-95 duration-150 relative"
