...
import { X as CloseIcon, Ticket, Calendar, Clock, CreditCard, ExternalLink, Loader2, Users, Phone, User, FileText } from 'lucide-react';
...
export default function BookingDetailsModal({ isOpen, onClose, publicId }: BookingDetailsModalProps) {
  ...
  const handleMarkFullyPaid = async () => {
    ...
    try {
      await apiClient.post(`/api/v1/admin/bookings/${booking.id}/mark-balance-paid`);
      toast.success('Booking marked as fully paid successfully.');
      window.location.reload();
    } catch (err: any) {
      toast.error(err.response?.data?.detail || 'Failed to mark as fully paid');
    } finally {
  ...
  const statusCfg = booking ? (STATUS_STYLE[booking.status.toUpperCase()] ?? {
            ...
            <div className="flex flex-col max-h-[85vh]">
                ...
                <div>
                  ...
                  <div className="bg-slate-50/40 border border-slate-100 rounded-2xl p-5 space-y-4">
                    ...
                    <div className="border-t border-slate-100 pt-4 flex justify-between items-center text-slate-800">
                      ...
                      <span className="text-lg font-black text-[#0f3d56]">
                    ...
                    {booking.remaining_balance > 0 && (
                      <div className="bg-amber-50 border border-amber-100 text-amber-800 rounded-xl p-3 flex justify-between items-center text-xs font-bold">
                        ...
                        <span className="text-sm font-black">{formatCurrency(booking.remaining_balance)}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="border-t border-slate-100 p-6 flex flex-wrap items-center justify-between gap-3 bg-slate-50/50">
                <div className="flex gap-2 flex-wrap">
                  ...
                  {booking.status === 'PARTIAL_PAID' && user?.role === 'ADMIN' && (
                    <button
